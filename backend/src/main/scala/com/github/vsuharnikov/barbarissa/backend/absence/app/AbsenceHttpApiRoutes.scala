...
object AbsenceHttpApiRoutes extends Serializable {
  ...
  val live = ZLayer.fromServices[AbsenceRepo.Service, AbsenceReasonRepo.Service, Service] { (absenceRepo, absenceReasonRepo) =>
    new Service with TapirCommonEntities {
      ...
      val getAllByEmployee = endpoint.get
      ...
      val getAllByEmployeeRoute: HttpRoutes[Task] = getAllByEmployee.toRoutes {
      ...
      val getRoute = get.toRoutes { absenceId =>
        for {
      ...
      override val http4sRoutes = List(getAllByEmployeeRoute, getRoute)
    }
  }
}
